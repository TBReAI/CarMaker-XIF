...
App_TestRun_Start (void *arg)
{
    ...
    // Surrounding_Allocate() has to be called after all sensors to finalize the initialization
	...
	if (SimCore.NextState == SCState_StartSim) {
		...
		if (SimCore.TimeWC - SimCore.Start.TimeWC > SimCore.Start.TimeLimit) {
		    LogErrF (EC_General, "Start hasn't finished within %g seconds."
			     " Test Run aborted.", SimCore.Start.TimeLimit);
		    SimCore_State_Set (SCState_End);
		}
	    }
	}
...
/***************************************************************
** MARK: NONSTANDARD CM_MAIN STARTS HERE 
***************************************************************/

#include "CM_Main.h"

static unsigned long long CycleNo64 = 0;
...
//extern beam_entry_t beam_table[];
...
bool CM_Main_running(void)
{
    ...
    return (
            (MainThread_BeginCycle(CycleNo64) == 0)
